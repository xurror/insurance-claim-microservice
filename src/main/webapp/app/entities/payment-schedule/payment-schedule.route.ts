import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { IPaymentSchedule, PaymentSchedule } from 'app/shared/model/payment-schedule.model';
import { PaymentScheduleService } from './payment-schedule.service';
import { PaymentScheduleComponent } from './payment-schedule.component';
import { PaymentScheduleDetailComponent } from './payment-schedule-detail.component';
import { PaymentScheduleUpdateComponent } from './payment-schedule-update.component';

@Injectable({ providedIn: 'root' })
export class PaymentScheduleResolve implements Resolve<IPaymentSchedule> {
  constructor(private service: PaymentScheduleService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IPaymentSchedule> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((paymentSchedule: HttpResponse<PaymentSchedule>) => {
          if (paymentSchedule.body) {
            return of(paymentSchedule.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new PaymentSchedule());
  }
}

export const paymentScheduleRoute: Routes = [
  {
    path: '',
    component: PaymentScheduleComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'insuranceClaimMicroserviceApp.paymentSchedule.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/view',
    component: PaymentScheduleDetailComponent,
    resolve: {
      paymentSchedule: PaymentScheduleResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'insuranceClaimMicroserviceApp.paymentSchedule.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: 'new',
    component: PaymentScheduleUpdateComponent,
    resolve: {
      paymentSchedule: PaymentScheduleResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'insuranceClaimMicroserviceApp.paymentSchedule.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/edit',
    component: PaymentScheduleUpdateComponent,
    resolve: {
      paymentSchedule: PaymentScheduleResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'insuranceClaimMicroserviceApp.paymentSchedule.home.title'
    },
    canActivate: [UserRouteAccessService]
  }
];
